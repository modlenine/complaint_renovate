...
class Dashboard_model extends CI_Model{
    ...
    public function getby_username(){
        return $this->db->query("SELECT Count(complaint_main.cp_user_name) AS num_user, complaint_main.cp_user_name FROM complaint_main GROUP BY cp_user_name ASC");
    }
    ...
    public function getby_topic(){
        return $this->db->query("SELECT complaint_main.cp_topic , COUNT(cp_topic) as num_topic FROM complaint_main GROUP BY cp_topic ASC");
    }
    
    public function viewcpby_status($cp_status_code){
        $result = $this->db->query("SELECT * FROM complaint_main WHERE cp_status_code='$cp_status_code' ");
