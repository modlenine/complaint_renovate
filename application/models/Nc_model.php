...
class Nc_model extends CI_Model{
    ...
    public function list_nc_from_cp($cpformno)
    {
        if($cpformno != ""){
            return $this->db->query("SELECT
            nc_main.nc_no,
    ...
    public function save_sec4f1($cp_no,$nc_related_dept){
        ...
        if($this->input->post("nc_sec4f1_status") == "no"){
            if($this->input->post('datetime41') == ""){
                    echo "<script>";
                    echo "alert('กรุณาระบุวันที่การติดตามผลครั้งที่ 2 ด้วยค่ะ');";
                    // echo "window.history.back(-1)";
                    echo "</script>";
                    header("refresh:0; url=".base_url('nc/main/').$cp_no."/".$nc_related_dept);
                    exit();
                }
        }
    ...
    public function save_sec4f2($cp_no,$nc_related_dept){
        ...
        if($this->input->post("nc_sec4f2_status") == "no"){
            if($this->input->post('datetime42') == ""){
                echo "<script>";
                echo "alert('กรุณาระบุวันที่การติดตามผลครั้งที่ 3 ด้วยค่ะ');";
                // echo "window.history.back(-1)";
                echo "</script>";
                header("refresh:0; url=".base_url('nc/main/').$cp_no."/".$nc_related_dept);
                exit();
    ...
    public function save_sec4f3($cp_no,$nc_related_dept){
        ...
        if($this->input->post("nc_sec4f3_status")=="no"){
            $linkurl = "complaint/add_failed/$cp_no/$nc_related_dept";
        ...
        //****************************Email***Zone*********************************************//
        $getEmail = $this->db->query("SELECT 
        maillist.deptcode, maillist.email, complaint_department.cp_dept_cp_no 
        FROM complaint_department 
        INNER JOIN maillist ON maillist.deptcode = complaint_department.cp_dept_code 
        WHERE cp_dept_cp_no = '$cp_no' && cp_dept_code = '$nc_related_dept' && cp_mail_status='1' ");
